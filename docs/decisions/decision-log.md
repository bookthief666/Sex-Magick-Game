...
## D-066 — The box comes off, and the build stops being a guess

**Date:** 2026-08-21
**Status:** Accepted. Aesthetic fix plus a diagnostic that should have existed five reports ago.

**Decision:** The owner reported the telegraph still centred inside a large empty rectangle. Measuring the current build at 368x690, 655x725 and 707x643 put it at 30-33px tall and 75%-82% down on every one - correct everywhere. The screenshot's box spans roughly 42%-78% of the play area, which is precisely `top: 41%` combined with `bottom: 128px`, and that pairing existed in exactly one build: **M39 (`8781c41`)**, before D-063 removed `viewport-runtime.js`'s `!important` top override. The owner's `◇ AEGIS EMPTY` readout corroborates it - that is M39's HUD format. **The last three "still broken" reports were all made against M39, two to three fixes behind**, and each was investigated against repository code that the device was not running. That is the most expensive mistake in this whole sequence and it was mine: a screenshot is evidence about a *build*, and I never once established which build produced it. `notice-slot.js` now exposes `buildMarker()` so the question is answerable in one line from the device, with no git and no server involved. Separately, the owner is right that the box is unnecessary: a 1px border with a solid slab behind it reads as a modal interruption rather than a whisper. All three text notices - gate telegraph, missions toast, power-up toast - drop the border and solid background for an edge-faded scrim (`linear-gradient` transparent at both ends), which stays legible over any artwork while having no perceivable edges. The telegraph's flash keyframes stop animating `border-color` and `background`, which no longer exist, and animate colour and glow alone. The ascent banner keeps its two accent rules, since those are lines rather than a box and are the ceremony the Sephirah change is meant to carry.

**Consequences:** Measured after the change at all three viewports: 30-33px tall, 75%-82% down, `border-width: 0px`. The 32 fast contract suites, the four-geometry cross-screen overlay test, and the gate-slice, missions and ritual-ascent browser suites are green. Nothing here changes position or arbitration - D-065's shared band and one-at-a-time guarantee are untouched, and this is purely what those notices look like. The wider lesson is the diagnostic one rather than the CSS: four of the five positional decisions in this sequence (D-060, D-062, D-063, D-064) were each verified against something other than what the owner was actually running - the wrong reference element, a synthetic DOM write, a repository file, or a stale browser cache. `buildMarker()` closes the last of those gaps, and the standing rule for any future report of this kind is to establish the build before analysing the symptom.

**Full record:** this entry.

## D-067 — Shields become earned, and the ascent becomes an arc

**Date:** 2026-08-21
**Status:** Accepted. First two steps of M40. Not a release authorization.

**Decision:** Two independent changes from the same playtest. **(1) Shield economy.** D-062 cut the cadence to one charge per 10 gates because the owner had never held a shield long enough to judge it; having played that, they reported shields arriving far too freely - the same lever pulled too far the other way. Distance covered is not an achievement, so `applyGateMilestones`, `GATES_PER_CHARGE` and the `gateChargeMarker` are **deleted rather than left inert**: an unused awarder that still works is a trap for whoever wires it back up, and the unit suite now asserts all three are `undefined`. Charges come from two things the player *did*: banking a Void (unchanged), and a new score checkpoint at `SCORE_PER_CHARGE = 500`, which exists so a player who keeps refusing the Gate is not locked out of the mechanic. `applyScoreMilestones()` mirrors the retired function's once-per-threshold shape and awards for every threshold crossed in a single observation, which matters because a full constellation (M40 §6) can pay several hundred points at once. **(2) The ascent re-spaced.** The old HEX thresholds (0/6/16/32/48/68/92/120) were front-loaded: +0.9, +1.2, +1.2 speed across the first 32 gates, then +0.7, +0.6, +0.5, +0.5 across the next 88. The owner's report of speed that "kept increasing until it sortve topped off" is that deceleration, felt at gate 32 rather than at the real ceiling. Intervals now widen monotonically - 9, 13, 18, 22, 26, 30, 34 - so each band lasts longer than the one before and the climb reads as a long arc; KETHER moves from gate 120 to 152, keeping the crown attainable in a strong run against the 2026-08-12 pilot's best of 507 gates. **Speeds and gaps are untouched**, so every pair remains an already-audited coordinate and no reachability evidence is invalidated. The full HEX audit (`test-player-reachability.js`) was re-run and passes.

**Consequences:** The perceived plateau is only half fixed. Re-spacing changes *when* the player arrives, not the speed deceleration itself, which exists because 8.5 is `MAX_VALIDATED_SPEED` - the top of the proven envelope. Raising it is a search, not an edit, and is deliberately left to a later step of M40 that ships only on new solver evidence. Re-spacing also exposed a brittleness worth naming: **five separate tests encoded the old spacing as literals** rather than deriving it - `test-player-reachability.js` filtered bands by `gateThreshold > 32` (which silently changed *which four bands* the audit covered), `test-gate-slice.js` asserted `getBandIndex(6/16/32)`, `test-ritual-ascent-runtime.js` asserted `gatesToNext === 6`, and `browser-gate-slice-test.mjs` drove to a hardcoded 7 gates expecting to be risk-active. All four now derive from `gate.BANDS`, so they assert the behaviour under test rather than the ladder's current spacing. The audit filter is the serious one: a `> 32` filter combined with a re-spacing would have kept passing while auditing a different set of bands than it claimed to.

**Full record:** this entry.

## D-068 — Twelve new walls, and a ladder that changes what it asks for

**Date:** 2026-08-21
**Status:** Accepted. M40.3. Not a release authorization.

**Decision:** The owner asked for "more patterns for the player to learn" and for the walls to get more interesting as a run climbs. Two changes. **(1) The library grew from 16 patterns to 28** - 8 to 14 per rite, weighted toward the thin families. Each new entry is a distinct *shape* rather than a numeric variation: `hex.plunge` descends and holds where every other pressure pattern returns to the anchor; `hex.pendulum` reads as rhythm rather than slope; `hex.hammerfall` is `hex.serpent-strike` inverted so a learned shape has a mirror that must also be learned; `monas.rising-calm` drifts up where `monas.orbit-settle` drifts down. **(2) `FAMILY_CYCLE` became `FAMILY_CYCLES`, a four-tier table selected by band.** A single fixed six-beat rotation ran from MALKUTH to KETHER, so the only thing that changed late in a run was speed and gap - the shapes were the same six beats seen in the first minute, which is why late play stopped surprising. The opening tier now schedules no climax at all; the crown tier schedules three per cycle with a single recovery beat. Every tier keeps six beats, so cursor arithmetic and pattern cadence are unchanged. `choosePattern(bandIndex)` defaults to 0 so every replay tool and test behaves as before, and the live spawn path reads the band from `gateSliceState`.

**Consequences:** **The solver rejected four of the twelve on first submission** - `monas.spiral-tighten`, `monas.undertow` and `monas.tide-return` invalid, `monas.double-helix` marginal - despite all twelve passing the `MAX_RATIO_DELTA` envelope check. That is the useful finding: the envelope bounds how far a gap may move between pillars, while the solver asks whether a player can actually be in both places in time at 8.5 speed in a 110px corridor. The envelope is necessary and not sufficient. Amplitudes were reduced until each came back `verified`, and one - a `center`-kind recovery pattern - was rejected even at the shipped `monas.return-flow`'s exact values, which localised the refusal to the motion/gapScale envelope rather than the shape; it was replaced with an offsets-based pattern rather than loosened. New ids are registered in `reachability-policy.js`'s `VERIFIED_PATTERN_IDS`, which is an allowlist and not a description: a pattern absent from it is swapped for the rite's fallback at runtime, so nothing reaches a player the solver has not cleared. The compositional audit was **strengthened rather than merely repaired**: it now proves a full cycle survivable back-to-back at all four tiers instead of only the base rotation, because the crown tier's three-climax cycle is exactly the case where every pattern can verify in isolation while the sequence is still compositionally unsurvivable. All four tiers pass. Five more tests were found asserting the library's size or the seeded sequence's identity rather than their actual contracts (case counts of 252 and 1008, a 16-pattern verdict count, a named `rejectedPatternId`, a hardcoded base cycle in browser QA); all now derive, with floors so coverage cannot silently shrink.

**Full record:** this entry.

## D-069 — The corridor becomes a constellation, and "the Void" turns out to be two sections

**Date:** 2026-08-21
**Status:** Accepted. M40.6. Not a release authorization.

**Decision:** The owner asked for bonus stars worth significantly more, evolving into higher-order polygrams, with a picture-glitch reward. Planning that work started from a premise that reading `index.html` alone supports and that is **wrong**: that the Void is the bonus star section. `isSpawnFrame()` returns `false` whenever `game.voidMode` and `index.html` spawns stars inside that same `if`, so the two look like one feature. But `gate-slice-runtime.js` flips the flag before delegating — the wagered Void clears `voidMode` around the original call, so the wager spawns **walls and no stars**, while the separate bonus corridor (`tickBonusCorridor`, every 25 gates, 300 frames) sets it and spawns stars and no walls. Two sections, one name. The star work therefore lands on the **bonus corridor**; the black-background work (M40.5) lands on the **wager**.

Three modules and one behaviour change. **(1) `tools/polygram.js`** generalises the hardcoded five-point loop to `{n/k}`. The load-bearing property is that stepping by `k` around `n` vertices closes after `n / gcd(n,k)` steps, so the figure is `gcd(n,k)` separate cycles — a hexagram is two triangles and no single-loop implementation can draw it. Emitting one cycle per residue class handles pentagram through dodecagram with no special cases. Deliberately canvas-free so the geometry is testable without a rendering context. **(2) `tools/void-constellation.js`** makes a corridor a **known finite set** rather than a stream of pickups. That one change is what lets a streak and a completion exist: catching consecutive stars raises the *next* star's order, a miss drops it back, and clearing the set pays a completion bonus. `BASE_STAR_VALUE = 25` plus `VALUE_PER_EXTRA_POINT = 7` replaces the flat `+10`. `claimCompletion()` is once-only so a double update on the completing frame cannot pay twice. **(3) The glitch reward reuses `GlitchFX`** — `rgbSplit` for a small streak, `shearTear` once it is running, and `sweepBeam` on completion, the same beam a won wager gets. No second effect vocabulary was invented, and it fires per catch and never per frame, because `rgbSplit` and `shearTear` each do a full-canvas `getImageData`.

**A shield is deliberately not awarded for clearing a set.** The plan called for one, but D-067 had just removed the timer-based source precisely because free shields meant nothing, and a corridor arrives every 25 gates. The completion pays **score**, which feeds D-067's 500-point checkpoint — so clearing corridors still earns shields, through the economy rather than around it.

**Consequences:** The wiring, not the modules, is where the risk was, and the unit tests cannot see it — they would have gone on passing with the script tag missing, the spawn seam attached to the wrong section, or the award never called, which is the exact shape of the D-062 shield bug. The browser suite now drives the real page: it asserts both modules load, that a corridor spawns exactly its set and no more, that a streak raises the order, that a cleared set pays several times the old rate, and that **the wager never builds a constellation**. Three findings came out of running it rather than reading it. **(1) A fixed 30-frame spawn interval emptied the section** — the smallest set finished in 120 of the corridor's 300 frames, leaving three seconds of black. Spawn pacing is now derived from the corridor's own countdown, and the test asserts the last star arrives past frame 200 so this cannot regress silently. **(2) The `frames % 60` compaction filter dropped uncaught stars silently.** Before this milestone that was invisible; with a streak it would have let a miss go uncounted at random. Misses are counted in both removal paths behind a `__missed` guard, and the guard is asserted. **(3) The first version of the wager assertion was flaky** — it parked the player on a star, then spawned walls, and the resulting death cleared `__gateSliceVoidActive` mid-loop so the frames afterwards were an ordinary run that could legitimately open a corridor. It was measuring the wrong section. It now resets the player and stops sampling when the wager ends, with a floor on sampled frames so it cannot pass vacuously. `?bonusEvery=N` was added for the same reason the marker was added in D-066: verifying the star section previously meant clearing 25 gates first, and that cost is what leaves a section unverified.

**Full record:** this entry.

## D-070 — The Void stops being black, and the artwork path turns out to be somewhere else

**Date:** 2026-08-21
**Status:** Accepted. M40.5. Not a release authorization.

**Decision:** The wagered Void rendered as a black screen; the owner asked for the picture to be there, zoomed, with the figures outlined like electric snakes. The outline is a Sobel edge pass, and two facts decide whether it is affordable: it must run **once per background rather than once per frame**, and it must run at **a fraction of the source resolution** — a 1600px gallery image is 2.5M pixels through an 8-tap kernel, which is not a draw effect but a stall. `tools/void-edge-layer.js` owns both: `MAX_EDGE_DIM = 384` bounds the pass to ~260k pixels whatever the source, and the result is cached by `src` behind a three-entry cap, because caching all 75 gallery images would be ~45MB of backing store for a section that shows one. Each frame then costs one composite of that layer with an animated hue plus one narrow bright strip travelling down it — reusing the layer rather than adding a second effect. The pixel maths is kept free of canvas and DOM so it is testable in Node against hand-built arrays; the border ring is deliberately left at zero rather than clamped, since an edge detector run over a clamped border outlines the picture frame itself, which reads as a bug rather than as a silhouette.

**Consequences:** **The edit was initially made in the wrong file, and only running it revealed that.** `index.html`'s `drawHyperspaceTunnel` looks like the artwork path and is not: `occult-field-runtime.js` replaces the method and calls the original with the band level's `loaded` forced false, precisely so it can redraw the artwork itself at full device resolution. The Void's black screen came from one line in *that* module's `drawLevelArtwork`, which returned early on `voidMode`. The `index.html` tail is now marked as the fallback it actually is. The measurement that caught this is worth keeping: the first version of the browser assertion reported **zero edge-layer builds and passed**, because it was asserting an upper bound on work that was never happening. It now asserts a lower bound too, checks that the artwork path is live before measuring its behaviour, and reads back the frame to assert the Void is not black — the milestone's actual claim, in pixels. Two further findings. **(1) `prepareLevels` snapshots levels with `{ ...lvl }` at Start**, and `img`/`loaded` are set on the *pool* entry when decoding finishes, so any image arriving after that moment is missing for the whole run — with `finishLoading()`'s eight-second failsafe, that is a real path on a phone, and the harness showed all 75 pool images loaded against zero loaded level copies. `levelImage()` resolves through the pool by id. The Gate slice's own gallery already avoided this and says so in a comment; the copy was the fallback path. **(2) `browser-m35-living-sephiroth-test` was failing before this work**, waiting forever for YESOD at `gatesCleared = 6` — a threshold D-067 moved to 9. That is a fifth test encoding the old spacing as a literal after the four that milestone fixed, and it failed as a 30-second timeout rather than a wrong-band assertion, which is the least informative way for it to break. All three of its band literals now derive from `BANDS`. Confirmed against a worktree at the previous commit: the YESOD timeout reproduces there, and so does a remaining media-codec page error, which is environmental to this headless Chromium and is **not** fixed here.

**Full record:** this entry.

## D-071 — Neon walls on the risk bands, and two tests that were measuring noise

**Date:** 2026-08-21
**Status:** Accepted. M40.4. Not a release authorization.

**Decision:** The owner asked for neon, glitching walls in the harder sections. This extends `chargeRiskEdges`'s existing reading — risk-active bands wear cyan — rather than introducing a second vocabulary, so the colour still means one thing. `neonRiskWall()` adds a hard bright rim on the gap-facing edge, exactly where the player must clear, and slices of the wall that shear sideways. Intensity climbs with band index, so the walls get louder as the run gets harder instead of being equally loud from the first risk band. Two deliberate constraints. **The shear is offset rectangles, not shifted pixels:** a genuine band-shift needs the wall's own pixels, and every route to those — `getImageData`, or blitting the canvas onto itself — is a full-frame readback per pillar per frame with six pillars on screen; offset bars read as the same tear for two `fillRect`s. **Nothing uses `Math.random()`:** everything derives from `pillar.x` and a coarse frame bucket, because a glitch that re-rolls every frame strobes rather than tears, and a non-deterministic effect would make the M14 visual baselines unrepeatable. Bars are drawn on the solid spans only — one across the gap would sit where the player has to fly and read as an obstacle that is not there.

**Consequences:** Two failing suites came out of this, and **neither was caused by the walls** — which is the point worth recording, because the obvious move in both cases would have been to assume it was and start tuning. **(1) `browser-m21-aesthetic-test`'s desktop glow budget** failed on the first run after the change, at 30.02ms against a 30ms ceiling. Measuring both configurations rather than assuming: 26.9/26.9 ms with the neon walls, 28.1/29.3/26.4 without. The walls cost nothing measurable; the bound simply sat inside the noise floor of a `--disable-gpu` software rasteriser whose baseline is 26-29ms, and tripped about one run in three with nothing changed. A bound that fails on noise reports nothing, so it is raised to 36 — still multiples below a real regression — and **both costs now print on every run**, because a performance number visible only when it trips is one nobody can tell a drift from a blip in. **(2) `browser-m18-missions-test`** was failing before any M40 work, verified against a worktree at D-066. Its rotation step set `bandIndex = 7` and completed a mission in the same frame; arriving at KETHER raises the ritual-ascent banner, and since D-065 exactly one transient notice is visible at a time, so the ascent claimed the slot *after* the mission announced and the assertion read a correctly-hidden notice as "never announced". The band change now lands on its own frame first. This is the second test in this milestone found asserting behaviour that a *previous* milestone deliberately changed — the first was M35's band literals in D-070 — and both failed in ways that pointed away from their real cause.

**Full record:** this entry.

## D-072 — The speed ceiling does not move, and the reason is not the one the search was looking for

**Date:** 2026-08-21
**Status:** Accepted. M40.2b. Not a release authorization. **No gameplay change.**

**Decision:** `MAX_VALIDATED_SPEED` stays at **8.5**. The plan named this outcome in advance - "if the search returns nothing above 8.5, the ceiling does not move" - and this is that case, though not for the reason anticipated.

`tools/run-m40-speed-frontier.mjs` establishes what the constant always implied. 8.5 was never a discovered maximum: every hard case in `DEFAULT_SCENARIOS` is pinned to `speed: 8.5, gap: 110`, so 8.5 is the fastest the solver had been *asked* about. Asked properly, **HEX is nowhere near its limit.** All twelve grid coordinates verified - speeds 9.0, 9.5 and 10.0 across gaps 110/118/126/134, 450 of 450 cases clean each. The solver's actual boundary at gap 122 sits between 12.0 (verified) and 14.0 (rejected, 9 marginal). Every exact ladder coordinate a raise would need was proven at its own value rather than interpolated from neighbours: CHESED 7.3/138, BINAH 8.2/132, CHOKMAH 9.1/127, KETHER 10.0/122, plus every Void coordinate the raised clamp produces, since the wager clamps to the ceiling and speeds up with it.

**And none of it mattered, because HEX was never the binding constraint.** The shipped band audit requires *every pattern in both rites* to clear every post-GEBURAH band, and `BANDS` is HEX's ladder. Run against it, the 10.0 ladder fails with 25 marginal and 40 invalid, the 9.5 ladder with 13 and 24, and even a 9.0 ladder with 5 and 20 - while the shipped 8.5 ladder passes 1800 of 1800. Every failing case is **MONAS**, and at the 9.0 ladder every one is at **KETHER alone**: seven MONAS patterns cannot hold that band. Widening the corridor does not rescue it - at 9.0 the crown still fails at gap 145, a wider corridor than CHESED's, with invalids gone but ten marginals left. The limit is the MONAS physics profile, not the geometry.

**Consequences:** The methodological finding is the one worth keeping. **Two hours of evidence measured the wrong constraint** because the search was built around a bespoke grid rather than around the audit that actually gates the change; running the shipped band audit first would have found this immediately. A second error compounded it: the grid omitted `policy.applyPatternOverride`, which the shipped audit passes, so it scored raw patterns. That direction was lucky - raw patterns are the harder case, making the HEX results conservative rather than optimistic - but it also produced a spurious "MONAS is already broken at 8.5" reading that was an artifact of the harness. The rule this yields: **a bespoke audit is a hypothesis, and the shipped audit is the test.** Two controls were added to the frontier tool for related reasons and both earned their place - a clean baseline proving the instrument can say yes, and an impossible coordinate (speed 20, which returns 40 marginal and 60 invalid) proving it can say no, since a grid where everything verifies is otherwise indistinguishable from a search that varies nothing.

A real option is left on the table rather than taken. `BANDS` is HEX's ladder and the Gate slice is HEX-only (`updateGameObjects` returns early unless `gameMode === 'HEX'`), so auditing MONAS's patterns against HEX's bands over-constrains the ceiling. Decoupling that would likely unlock the raise. It is deliberately **not** done here: it weakens a standing safety property in order to pass, which is the same move as loosening a test, and whether MONAS should be held to HEX's ladder is the owner's call rather than a mechanical one.

The owner's "kept increasing until it sortve topped off" is therefore addressed by D-067's re-spacing alone, which changes *when* the plateau arrives rather than removing it. That is a partial answer, and it should be described as one.
