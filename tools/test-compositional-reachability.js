...
for (const rite of ['HEX', 'MONAS']) {
  ...
  assert.equal(solution.solvable, true, `${rite} full family cycle was not compositionally reachable`);
  assert.equal(solution.witnessValid, true, `${rite} witness did not replay exactly`);
  assert.equal(solution.boundaryResults.length, grammar.FAMILY_CYCLE.length);
